...
################################################################  
# THIS CONTAINS PROGRAM TOOLS, NOT ANYTHING FOR THE GAME ITSELF
################################################################
...
def handSorter(cards):
    ...
    while i < len(cards) and j != len(cards):
        ...
        if i != 0:
            cardScoreBelow = cards[i-1][:len(cards[i])-1]
        if cardScore.isdigit() and i > 0 and (cardScoreBelow.isdigit() and int(cardScore) > int(cardScoreBelow)):
            x = cards[i]
            cards.remove(x)
            cards.insert(len(cards), x)
            i -= 1
        elif cardScore == "J" or cardScore == 'Q' or cardScore == 'K':
            x = cards[i]
            cards.remove(x)
            cards.insert(len(cards), x)
            i -= 1
...
def Start(reply):
    ...
    time.sleep(2)
    print("\n\t   I am 5.5 hours into making this...")
    time.sleep(0.5)
